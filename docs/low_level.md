...
### DefineMultiple(*, feature, first_id, count, props)
- first_id - The ID of first entity to define.
- count - Amount of entities to define.
...
### AdvancedSpriteLayout(*, ground, buildings=(), has_flags=True, feature=None, ref_id=None)
- ground - Ground `Sprite`.
- buildigns - List of building `Sprite`s.
- has_flags - TODO

### Switch(*, ranges, default, code, *, feature=None, ref_id=None, related_scope=False)
...
### RandomSwitch(*, scope, triggerss, cmp_all, lowest_bit, groups, count=None, feature=None, ref_id=None)
...
### Map(object, maps, default)
- object - entity to map graphics for
- maps - cargo type to ref ID dict
- default - default ref ID
...
### If(*, is_static, variable, varsize, condition, value, skip)
...
### Comment(data)
- data - content of the comment, has no effect on the GRF functioning.
